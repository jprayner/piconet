...
describe('objectInfo protocol handler', () => {
  ...
  it('should read directory object info successfully', async () => {
    ...
    waitForReceiveTxEventMock.mockImplementation(async (station: number, controlByte: number, replyPorts: number[]) => {
  ...
  it('should throw error if server fails to respond to read directory object info', async () => {
    ...
    await expect(readDirAccessObjectInfo(254, '$')).rejects.toThrowError('Failed to send object info command (0x12) to station 254: Something bad happened');
  });
  ...
  it('should throw error if server returns malformed response from read directory object info', async () => {
    ...
    await expect(readDirAccessObjectInfo(254, '$')).rejects.toThrowError('Malformed response from station 254: success but not enough data');
  });
});
