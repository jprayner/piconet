...
export const iAm = async (serverStation: number, username: string, password: string) => {
  const serverReply = await executeCliCommand(serverStation, `I AM ${username} ${password}`);
