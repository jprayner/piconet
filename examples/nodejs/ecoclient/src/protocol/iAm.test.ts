...
describe('iAm protocol handler', () => {
  ...
  it(`should call executeCliCommand with the correct arguments`, async () => {
    ...
    expect(executeCliCommandMock).toHaveBeenCalledWith(254, 'I AM JPR93 MYPASS');
  ...
  it(`should call throw error on unexpected server reply length`, async () => {
    ...
    await expect(iAm(254, 'JPR93', 'MYPASS')).rejects.toThrowError('Malformed response from station 254: success but not enough data');
  });
});
