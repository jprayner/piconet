...
describe('status message parser', () => {
  it('should parse valid string', () => {
    ...
    expect(result?.boardState).toBe(BoardState.Listen);
  });

  it('should return no match (undefined) due to empty string', () => {
    const eventStr = '';
    expect(parseEvent(eventStr)).toBeUndefined();
  });
  ...
  it('should fail to parse due to no attributes', () => {
    ...
    expect(() => parseEvent(eventStr)).toThrow(`Protocol error. Invalid STATUS event '${eventStr}' received`);
  });
  ...
  it('should fail to parse due to incompatible board version', () => {
    ...
    expect(() => parseEvent(eventStr)).toThrow(`Driver version ${config.version} is not compatible with board version 99.9.9.`);
  });

  it('should fail to parse due to wrong number of attibutes', () => {
    const eventStr = `STATUS ${config.version} 32 10 1 i_shouldnt_be_here`;
    expect(() => parseEvent(eventStr)).toThrow(`Protocol error. Invalid STATUS event '${eventStr}' received. Expected 4 attributes, got 5`);
  });

  it('should fail to parse due to invalid econet station number', () => {
    const eventStr = `STATUS ${config.version} xxx 10 1`;
    expect(() => parseEvent(eventStr)).toThrow(`Protocol error. Invalid STATUS event '${eventStr}' received. Invalid econet station 'xxx'.`);
  });

  it('should fail to parse due to invalid status register value', () => {
    const eventStr = `STATUS ${config.version} 32 xxx 1`;
    expect(() => parseEvent(eventStr)).toThrow(`Protocol error. Invalid STATUS event '${eventStr}' received. Invalid status register 1 value 'xxx'.`);
  });
});
